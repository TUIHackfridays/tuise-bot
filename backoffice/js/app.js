...
function kickoff() {
  ...
  function localeCodeToEnglish(loc) {
    ...
    if (parts.length > 1)
        return (ISO639_1[parts[0]] || ISO639_2[parts[0]] || parts[0]) + ' from ' + (ISO3166_1[parts[1]] || parts[1]);
    if (parts.length > 0)
  ...
  function getBotSettings() {
    request('GET', '/bot-settings', null, function(resp) {
      if(resp.message) {
        resp.message.language = localeCodeToEnglish(resp.message.language);
        var template = $('#template-whoami').html();
        Mustache.parse(template);   // optional, speeds up future uses
        var rendered = Mustache.render(template, resp.message);
        $('.bot-settings').html(rendered);
        getAllBotSettings(resp.message.id);
      }
    });
  }

  function getBotLanguages() {
    request('GET', '/translation-locales', null, function(resp) {
      if(resp.message) {
        var template = $('#template-cando').html();
        Mustache.parse(template);   // optional, speeds up future uses
        var rendered = Mustache.render(template, {"languages": resp.message.join(', ')});
        $('.bot-cando').html(rendered);
      }
    });
  }

  function getAllBotSettings(selected) {
    request('GET', '/bot-settings-all', null, function(resp) {
      if(resp.message) {
        var re = /-[a-zA-Z]*/gi;
        resp.message = resp.message.map(function(data){
          var lang = "";
          if(data.language.match(re).length > 1) {
            lang = data.language.match(re)[1];
            data.language = data.language.replace(lang, '');
          }
          data.language = localeCodeToEnglish(data.language);
          data.language += localeCodeToEnglish(lang);
          return data;
        });
        var template = $('#template-settings').html();
        Mustache.parse(template);   // optional, speeds up future uses
        var rendered = Mustache.render(template, resp);
        $('#changebot').html(rendered);
        $('#changebot').val(selected);
      }
    });
  }

  function updateBotSettings(form) {
    var btn = $(form).find(".button-primary" );
    btn.toggleClass('in-progress');
    var data = { "settings": parseInt(form.changebot.value) };
    request('POST', '/bot-settings', data, function(resp) {
      if(resp.message) {
          document.getElementById('alert-msg').innerHTML = resp.message;
          getBotSettings();
          setTimeout(function(){
            btn.toggleClass('in-progress');
          }, 1000);
  ...
  function getAllResponses() {
    request('GET', '/chat-response', null, function(resp) {
      if(resp.message) {
        var template = $('#template-responses').html();
        Mustache.parse(template);   // optional, speeds up future uses
        var rendered = Mustache.render(template, resp);
        $('#selectResp').html(rendered);
      }
    });
  }

  function setResponse(form) {
    var btn = $(form).find(".button-primary" );
    btn.toggleClass('in-progress');
    var data = {
      "trigger": form.trigger.value,
      "response": form.newResp.value,
      "responseID": form.selectResp.value
    };
    request('POST', '/chat-response', data, function(resp) {
      if(resp.message) {
          document.getElementById('alert-msg').innerHTML = resp.message;
          getBotSettings();
          setTimeout(function(){
  ...
  this.getBotSettings = getBotSettings;
  this.getBotLanguages = getBotLanguages;
  this.getAllBotSettings = getAllBotSettings;
  this.updateBotSettings = updateBotSettings;
...
window.addEventListener('load', function() {
  ...
  $( window ).resize(function() {
    ...
    var speed = Math.floor(size * 100 / 1980);
    document.getElementsByClassName('alert')[0].style.webkitAnimationDuration = speed+"s";
  });
}, false);
